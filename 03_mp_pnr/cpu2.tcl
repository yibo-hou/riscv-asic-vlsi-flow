...
floorPlan -site CoreSite -s 250 150 10 10 10 10
...
set regfile_x1 82
set regfile_y 19.87
set regfile_pitch 5.6

for {set i 0} {$i < 32} {incr i} {
        set col [expr {$i / 16}]
        set row [expr {$i % 16}]
        set x [expr {$col == 0 ? $regfile_x0 : $regfile_x1}]
        set y [expr {$regfile_y + $row * $regfile_pitch}]
        placeInstance [format {datapath/bitslices[%d].bitslice/regfile} $i] $x $y R0
}

addHaloToBlock 1.4 1.4 1.4 1.4 -allMacro
...
# TODO add power grid
addStripe -nets {vss! vdd!} -layer metal6 -direction vertical -width 2 -spacing 2 -set_to_set_distance 20
...
editPin -pin * -side bottom -layer metal3 -spreadType center
...
# Set ECE425_INNOVUS_MAP locally before exporting GDSII.
streamOut cpu2.gdsii -mapFile $::env(ECE425_INNOVUS_MAP)
